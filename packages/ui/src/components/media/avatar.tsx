...
export const Avatar: UixComponent<"img", Assign<PropsOf<typeof AvatarPrimitive.Image>, AvatarProps>> = (props) => {
  ...
  const defaultFallback = (
    <LuUser className="inline-block size-[1em] min-h-[1lh] shrink-0 align-middle text-2xl text-current leading-[1em]" />
  );
  ...
  const avatarStyle: object = {
    ...
    zIndex: stacking
      ? stacking === "first-on-top"
        ? "calc(var(--avatar-group-count) - var(--avatar-group-index))"
        : "var(--avatar-group-index)"
      : undefined,
    ...style,
  };
  ...
  return (
    <AvatarPrimitive.Root asChild>
      <div
        className={cn(
          avatarVariants({ size }),
          "flex items-center justify-center bg-[var(--avatar-bg)] text-[var(--avatar-color)]",
          !name && "bg-gray-200",
          stacking && "-ms-3 border-2 border-white",
          className,
        )}
        style={avatarStyle}
      >
...
export const AvatarGroup: UixComponent<"div", AvatarGroupProps> = (props) => {
  const { children, className, max, size, stacking = "last-on-top", style, ...remainingProps } = props;
  ...
  return (
    <uix.div
      className={cn("relative isolate inline-flex items-center justify-start", className)}
