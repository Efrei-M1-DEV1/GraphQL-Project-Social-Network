import type { Dict, IncludeContainerType } from "./types";

const isHTMLElement = (element: unknown): element is HTMLElement =>
  typeof element === "object" && element !== null && (element as Dict).nodeType === 1;

const isFrame = (element: unknown): element is HTMLIFrameElement => isHTMLElement(element) && element.tagName === "IFRAME";

function isVisible(el: unknown) {
  if (!isHTMLElement(el)) {
    return false;
  }
...
export function getFirstFocusable(container: HTMLElement | null, includeContainer?: IncludeContainerType): HTMLElement | null {
 ...
 */
export function getFirstTabbable(container: HTMLElement | null, includeContainer?: IncludeContainerType): HTMLElement | null {
 ...
 */
export function getLastTabbable(container: HTMLElement | null, includeContainer?: IncludeContainerType): HTMLElement | null {
